...
class Cx extends TagLib {
    ...
    protected $tags   =  array(
		...
		'static'    =>  array('attr'=>'file,href,type,value,basepath','close'=>0,'alias'=>'load,css,js'),
    ...
    public function _import($tag,$content,$isFile=false,$type='') {
        ...
        if($isFile) {
            ...
            foreach ($array as $val){
                ...
                switch($type) {
                case 'js':
                    $parseStr .= '<script type="text/javascript" src="http://static'.DOMAIN.'/'.$val.'"></script>';
                    break;
                case 'css':
                    $parseStr .= '<link rel="stylesheet" type="text/css" href="http://static'.DOMAIN.'/'.$val.'" />';
                    break;
				case 'png':
                    $parseStr .= '<img src="http://static'.DOMAIN.'/'.$val.'">';
                    break;
                case 'php':
            ...
            foreach ($array as $val){
                ...
                switch($type) {
                case 'js':
                    $parseStr .= '<script type="text/javascript" src="http://static'.DOMAIN.'/'.$basepath.'/'.str_replace(array('.','#'), array('/','.'),$val).'.js'.($version?'?'.$version:'').'"></script>';
                    break;
                case 'css':
                    $parseStr .= '<link rel="stylesheet" type="text/css" href="http://static'.DOMAIN.'/'.$basepath.'/'.str_replace(array('.','#'), array('/','.'),$val).'.css'.($version?'?'.$version:'').'" />';
                    break;
				case 'png':
                    $parseStr .= '<img src="http://static'.DOMAIN.'/'.$basepath.'/'.str_replace(array('.','#'), array('/','.'),$val).'.png'.($version?'?'.$version:'').'">';break;
				case 'jpg':
                    $parseStr .= '<img src="http://static'.DOMAIN.'/'.$basepath.'/'.str_replace(array('.','#'), array('/','.'),$val).'.jpg'.($version?'?'.$version:'').'">'; break;
				case 'swf':
                    $parseStr .= 'http://static'.DOMAIN.'/'.$basepath.'/'.str_replace(array('.','#'), array('/','.'),$val).'.swf'.($version?'?'.$version:''); break;
                case 'php':
	...
	public function _static($tag,$content,$isFile=false,$type='') {
        $file       =   isset($tag['file'])?$tag['file']:$tag['href'];
        $parseStr   =   '';
        $endStr     =   '';
        // 判断是否存在加载条件 允许使用函数判断(默认为isset)
        ...
        if($isFile) {
            ...
            // 命名空间导入模式 默认是js
            $type       =   $type?$type:(!empty($tag['type'])?strtolower($tag['type']):'js');
            $basepath   =   !empty($tag['basepath'])?$tag['basepath']:__ROOT__.'/Game/Theme';
            // 命名空间方式导入外部文件
            $array      =   explode(',',$file);
            foreach ($array as $val){
                if(strpos ($val, '?')) {
