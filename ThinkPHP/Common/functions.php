...
function U($url='',$vars='',$suffix=true,$domain=true) {
    ...
    }elseif($domain===true && count(array_filter(explode('/',$url))) === 3){
    ...
    if($domain && $APP_SUB_DOMAIN_DEPLOY) {
		if(strpos($url,'/index/index.html')){
			$url   =  (is_ssl()?'https://':'http://').$domain.'/';
		}else if(strpos($url,'/index.html')){
			$url   =  (is_ssl()?'https://':'http://').$domain.str_replace("/index.html",".html",$url);
		}else{
			$url   =  (is_ssl()?'https://':'http://').$domain.$url;
		}
		unset($APP_SUB_DOMAIN_DEPLOY);
    }
