...
class LoginController extends AddonController{
    ...
    public function callback(){
        ...
        if($user_sync_info['uid'] && $user_sys_info['id'] && $user_sync_info['uid'] == $user_sys_info['id']) { //曾经绑定过
			...
			$_POST['reg_type']=3;
			$upload_data['path'] = $user_sns_info['head'];
			$upload_data['url'] = $user_sns_info['head'];
			$upload_data['ext'] = 'png';
			$upload_data['status'] = 1;
			$_POST['avatar'] = M('Upload')->add($upload_data);
			//$user_sns_info['name']=preg_replace("/[^a-zA-Z0-9_]+/","_", str_replace(" ","_",trim($user_sns_info['name'])));
			$user_sns_info['name']=trim(str_replace(" ","",trim($user_sns_info['name'])));
			if(strlen($user_sns_info['name'])<4 || !preg_match("/^(?!.*?_$)[\w\一-\龥]+$/",$user_sns_info['name'])){
				$user_sns_info['name']='t3t2_'.time();
			}
			$__temp=D('User')->where(Array('username'=>$user_sns_info['name']))->find();

			if(!empty($__temp)){
				$user_sns_info['name']='t3t2_'.time();
			}
			$username = $_POST['username'] = $user_sns_info['name'];
			$password = $_POST['password'] = 't3t2123!@#'.$_POST['avatar'];
			$user = D('User');
			$data = $user->create();
			if($data){
				$id = $user->add();
				$user->where('id='.$id)->save(Array('avatar'=>$_POST['avatar']));
				if($id){
					//新增SNS登录账号
