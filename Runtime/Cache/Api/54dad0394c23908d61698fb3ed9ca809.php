...
<link rel="stylesheet" type="text/css" href="http://static.yiwanshu.com//Public/theme/api/user.css" />
...
<?php if(isset($__USER__)): ?><div class="regbox" <?php if(!empty($_GET['bcolor'])): ?>style="background:#<?php echo ($_GET['bcolor']); ?>;"<?php endif; ?>>
        <div class="nam_box">
            <a href="<?php echo U('Api/User/logout','refrer=1');?>" class="tui_a" target="_self">退出登录</a>
            欢迎您：<a href="<?php echo U('User/Index/Index');?>"><?php echo ($__USER__["username"]); ?></a>
        </div>
        <div class="user_btn">
                <a href="<?php echo U('User/profile/index');?>">修改资料</a>
                <a href="<?php echo U('User/Safe/index');?>">设置密保</a>
                <a href="<?php echo U('User/Safe/index');?>">手机绑定</a>
        </div>
        <div class="user_fu">
            最近玩过：
        </div>
    ...
    type: "get",
    async: true,
    url: U("Api/User/Userplay","gid=<?php echo I('gid');?>"),
    dataType: "jsonp",
    jsonp: "callback",
...
<?php else: ?>
<!--登录前-->
    <div class="logbox" <?php if(!empty($_GET['bcolor'])): ?>style="background:#<?php echo ($_GET['bcolor']); ?>;"<?php endif; ?>>
    <form id="loginform" name="loginform" action="<?php echo U('Api/User/ajaxLogin');?>"  method="post" >
        <div class="formbox">
            <ul class="input_ul">
                <li>
				<input type="text" class="txt_css" value="请输入帐号" id="login-username" name="username" onblur="" onclick="if(this.value=='请输入帐号'){this.value=''}" onblur="checkuser_login(this)"></li>
                <li><input type="password" class="txt_css" value="******" id="login-password" name="password" onblur="checkpwd_login(this)" onclick="if(this.value=='******'){this.value=''}"></li>
            </ul>
            <input type="button" class="sub_css" onclick="submitLoginInfo()">
        </div>
    </form>
        <div class="login-tips" id="login-error"></div>
        <div class="user_a clearfix">
            <span><a href="<?php echo U('Service/ticket/password');?>">忘记密码?</a><a href="<?php echo U('User/Public/register');?>">注册帐号</a></span>
            <a href="<?php echo addons_url('SyncLogin://Login/login', array('type'=>'qq')); ?>" class="qq_btn" class="qq_btn">QQ登录</a>
        </div>
    </div>
<script type="text/javascript" src="http://static.yiwanshu.com//Public/theme/api/weblogin.js"></script><?php endif; ?>
	<script type="text/javascript" >
