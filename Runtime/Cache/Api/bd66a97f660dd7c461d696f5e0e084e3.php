<?php if (!defined('THINK_PATH')) exit();?><!DOCTYPE html>
<html>
<head>
<meta charset="utf-8">
<meta http-equiv="X-UA-Compatible" content="IE=7">
<link rel="stylesheet" type="text/css" href="http://static.yiwanshu.com//Public/theme/user/css/new_style.css" />
<script type="text/javascript" src="http://static.yiwanshu.com//Public/jquery/jquery183min.js"></script>
<script type="text/javascript" src="http://static.yiwanshu.com//Public/theme/user/js/main.js"></script>
<meta name="keywords" content="<?php echo ($meta_keywords); ?>">
<meta name="description" content="<?php echo ($meta_description); ?>">
<title><?php echo ($meta_title); ?>｜<?php echo C('WEB_SITE_TITLE');?>－<?php echo C('WEB_SITE_SLOGAN');?></title>
<script type="text/javascript" src="http://static.yiwanshu.com//Public/layer/layer.js"></script>
<script>
<?php echo W('Common/Public/script');?>
layer.config({
	skin:'layer-ext-moon',
	extend:'skin/moon/style.css'
});
</script>
<style>
#page-loader{
    position: fixed;
    top:0;
    left:0;
    bottom: 0;
    right: 0;
    background: #fff;
    z-index: 99999;
}

#page-loader .page-loader-gif{
    display: block;
    width:100%;
    height:64px;
    position: absolute;
    top:50%;
    left:0;
    margin:-32px 0 0;
    font-size:21px;
    line-height:58px;
    font-family:Raleway;
    font-weight:400;
    text-align: center;
    color:#aaa;
    color:rgba(0,0,0,0.2);
    background-image: url("data:image/gif;base64,");    
	background-repeat: no-repeat;
	background-position-x: 50%;
	background-position-y: 0px;
}
</style>
<base target="_blank">
</head>
<body>

<div id="page-loader"><span class="page-loader-gif">Loading...</span></div>

<div class="pop-box pop-login" id="pop-box">
		<a href="javascript:;" onclick="closeWrap();" title="关闭" target="_self" class="icon icon-x pop-close" id="pop-close"></a>
		<div class="pop-hd"> <i class="pop-hd-t"></i> <i class="pop-hd-b"></i> </div>
		<div class="pop-bd">
			<div class="pop-item pop-item-login" id="form_login" >
				<h2 class="title">登录</h2>
				<div class="pop-login-box">
					<form method="post" class="pop-login-form" id="loginform"  name="loginform" action="<?php echo U('Api/User/ajaxLogin');?>">
						
						<div class="w-form-item">
							<input type="text" id="login-username" name="username" placeholder="帐号" class="w-input-input" >
						</div>
						<div class="w-form-item">
							<input type="password"  id="login-password" name="password" placeholder="密码" class="w-input-input" >
						</div>
						<input type="button" value="登 录" onclick="submitLoginInfo()" class="w-input-submit w-login-btn" id="w-login-btn">
						<input type="checkbox" style="display:none;" id="remname" name="remname" checked>
					</form>
					<div class="m-login-ext fix">
					
						<div class="m-ext-left fl"> <i class="icon icon-checkbox icon-checkbox-selected">√</i> 自动登录 </div>
						<a href="javascript:switch_(1);" target="_blank" class="fr">立即注册</a>
					</div>
					<div class="m-login-extlogin">
						<div class="hd"><span>第三方登录</span></div>
						<div class="bd">
							<a href="<?php echo addons_url('SyncLogin://Login/login', array('type'=>'qq')); ?>" title="QQ帐号登录" target="_blank" class="i-thirdlogin i-thirdlogin-qq"></a>
						</div>
					</div>
				</div>
			</div>
			<div class="pop-item pop-item-login" id="form_reg" style="display:none;">
				<h2 class="title">注册</h2>
				<div class="pop-login-box">
					<form method="post" class="pop-login-form"  id="form1" name="form1"  action="<?php echo U('Api/User/ajaxRegister');?>">
						<div class="w-form-item">
							<input type="text" id="register-username"  name="username" placeholder="帐号" class="w-input-input" >
						</div>
						<div class="w-form-item">
							<input type="password" id="txtPassWord" name="password" placeholder="密码" class="w-input-input" >
						</div>
						<div class="w-form-item">
							<input type="password" id="txtRPassWord" name="repassword" placeholder="重复密码" class="w-input-input" >
						</div>
						<input type="button" value="注 册"  onclick="submitInfo()" class="w-input-submit w-login-btn" id="w-login-btn">
						<input type="hidden" name="do" value="login">
						<input type="checkbox" style="display:none;" id="acceptterms" name="acceptterms" checked>
					</form>
					<div class="m-login-ext fix">
						<div class="m-ext-left fl"> <i class="icon icon-checkbox icon-checkbox-selected">√</i> <a href="<?php echo U('Service/information/yhxy');?>">用户协议</a> </div>
						<a href="javascript:switch_(0);" target="_blank" class="fr">返回登录</a>
					</div>
					<div class="m-login-extlogin">
						<div class="hd"><span>第三方注册</span></div>
						<div class="bd">
							<a href="<?php echo addons_url('SyncLogin://Login/login', array('type'=>'qq')); ?>" title="QQ帐号登录" target="_blank" class="i-thirdlogin i-thirdlogin-qq"></a>
						</div>
					</div>
				</div>
			</div>
		
		
		</div>
	</div>

          
<script type="text/javascript" src="http://static.yiwanshu.com//Public/theme/user/js/logreg.js"></script>
<script type="text/javascript" src="http://static.yiwanshu.com//Public/theme/user/js/frame.js"></script>

<script>
	function showLoginError($input,errorMsg)
	{
		layer.alert(errorMsg,{title:'提示',icon: 2});
	}
	function showError($input,errorMsg)
	{
		layer.alert(errorMsg,{title:'提示',icon: 2});
	}

	function showSuccess($input,successMsg)
	{
		layer.alert(successMsg,{title:'提示',icon: 1})
	}
	function switch_(num){
		$("#form_login").toggle();
		$("#form_reg").toggle();
		if(num==1){
			$(".pop-bd").height(540);
		}else{
			$(".pop-bd").height(340);
		}
	
	
	}
$(document).ready(function() {
	$(window).load(function(){
		$('#page-loader').fadeOut(500);
	});
	var mod = location.hash;
	if(mod == "#signup"){
		switch_(1);
	}
  
});
</script>

</body>
</html>