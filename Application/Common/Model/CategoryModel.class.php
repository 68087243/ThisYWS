...
class CategoryModel extends Model{
     ...
     * @author jry <598821125@qq.com>
     */
    public function getCategoryTree($id = 0, $group = 1, $field = true, $post_auth , $num){
        //获取当前分类信息
        if((int)$id > 0){
            $info = $this->find($id);
            $id   = $info['id'];
		...
		$map['post_auth']  = $post_auth;
        $tree = new \Common\Util\Tree();
        $list = $this->field($field)->where($map)->limit($num)->order('sort')->select();
